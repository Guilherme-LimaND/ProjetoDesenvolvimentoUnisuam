document.addEventListener('DOMContentLoaded', function () {
  ...
  document.getElementById('form-register').addEventListener('submit', function (e) {
    ...
    var cpf      = document.getElementById('reg-cpf').value.trim();
    var cep      = document.getElementById('reg-cep').value.trim();
    ...
    if (cpf && cpf.replace(/\D/g,'').length !== 11) { setErr('err-reg-cpf','CPF deve ter 11 dígitos.'); ok=false; }
    if (cep && cep.replace(/\D/g,'').length !== 8)  { setErr('err-reg-cep','CEP deve ter 8 dígitos.');  ok=false; }
    ...
    setTimeout(function () {
      ...
      var res = Auth.registerUser(nome, email, telefone, senha, cpf, cep);
